...
{
	// Search first 16MB instead of 64KB
	UINT64 begin = 0x1000;
	UINT64 end = 0x1000000;
	BOOL bRes = FALSE;
	UINT64 val = 0;
...
{
	// Method 1: Use EnumDeviceDrivers (most reliable)
	LPVOID drivers[1024];
	DWORD cbNeeded;
	...
	// Method 2: Fallback to NtQuerySystemInformation
	UINT64 retval = 0;
...
UINT64 LenovoMemoryMgr::FindBase(const char* image_name)
{
	...
	if (EnumDeviceDrivers(drivers, sizeof(drivers), &cbNeeded))
	{
		...
		for (int i = 0; i < driverCount; i++)
		{
			...
			{
				if (strstr(driverName, image_name))
				{
...
BOOL LenovoMemoryMgr::Init(std::string driver_name)
{
	...
	(
		strDeviceName,
		GENERIC_READ | GENERIC_WRITE,  // Added GENERIC_WRITE
		FILE_SHARE_READ | FILE_SHARE_WRITE,  // Added FILE_SHARE_WRITE
		NULL,
		OPEN_EXISTING,
